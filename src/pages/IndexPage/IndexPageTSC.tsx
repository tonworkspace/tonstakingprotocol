...
import { performTwoWaySync, ScorpionMiner, syncDataFromSupabase, saveAndSyncScorpionMinerData  } from '@/playerSupabaseTCS';

import { openDB } from 'idb';
import axios from 'axios'; // Use axios to fetch referral data
...
export const IndexPage: FC = () => {
  ...
  useEffect(() => {
    const saveProgress = async () => {
      if (user && scorpionMiner) {
        // Merge the updated fields into a complete ScorpionMiner object
        const updatedScorpionMiner = {
          ...scorpionMiner, // Include all existing fields from scorpionMiner
          balance,
        ...
        console.log('Saving updated scorpion miner data:', updatedScorpionMiner); // Log data before saving
  
        // Pass the complete ScorpionMiner object to saveAndSyncScorpionMinerData
        await saveAndSyncScorpionMinerData(updatedScorpionMiner);
      }
    };
  ...
  const fetchReferralData = async () => {
    ...
    try {
      const response = await axios.get(`https://scorpion-server.glitch.me/referrals/${user.id}`);
      setReferralCount(response.data.referralCount);
      setReferrals(response.data.referrals || []); // Ensure it's always an array
    } catch (error) {
      console.error('Failed to fetch referral data:', error);
      setReferrals([]); // Set to empty array in case of error
    } finally {
